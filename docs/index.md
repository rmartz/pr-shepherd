...
- [Step Fan-Out / Fan-In](subsystems/step-fan-out-fan-in.md) — a step spawns N child steps and parks in `waiting`; the scheduler joins it once every child is terminal, aggregating their outputs — the scatter/gather primitive under the `github_api` batch split.
