...
- **Hard cap (enforced)**: a changed `.ts`/`.tsx` file at +100% of the recommended limit — **400** lines for source, **600** for tests — fails the file-length check. It is enforced both locally at commit time (`claude/hooks/pre-commit`, which runs in every worktree — including agent worktrees under `.git-worktrees/` — and dispatches to `.husky/pre-commit` in the primary checkout) and in CI (`file-length.yml`), via the shared `scripts/check-file-length.mjs`. `git commit --no-verify` bypasses the local hook; CI remains the backstop.
