...
import { StepStatus, type StepInstance, type WorkflowRun } from "@/db/schemas";
...
//
// Aggregation is a pure read-side rollup over `stepInstances` joined to their
// `workflowRuns` (which carry `repo` + `prNumber`); it never mutates state, so
// it is safe to call repeatedly during a drain loop and once more at exit.
// ---------------------------------------------------------------------------
...
// Roll the step set up into a per-PR drain summary. Steps whose run is missing
// are skipped — they cannot be attributed to a PR and a dangling step is an
// inconsistency a separate reconciliation pass owns, not the drain report.
export async function aggregateDrainResults(db: Db): Promise<DrainAggregate> {
  ...
  const runCache = new Map<string, WorkflowRun | undefined>();

  for (const step of steps) {
    let run = runCache.get(step.runId);
    if (run === undefined && !runCache.has(step.runId)) {
      run = await db.get(Collections.workflowRuns, step.runId);
      runCache.set(step.runId, run);
    }
    if (run === undefined) continue;
    tally(bucketFor(summaries, run), step);
  }
